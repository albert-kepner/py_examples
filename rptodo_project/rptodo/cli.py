...
def get_todoer() -> rptodo.Todoer:
    ...
    if db_path.exists():
        ...
        typer.secho(
            'Database file not found. Please, run "rptodo init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
...
def list_all() -> None:
    """Display a list of to-dos from the database"""
    todoer = get_todoer()
    todo_list = todoer.get_todo_list()
    if len(todo_list) == 0:
        typer.secho(
            "There are no to-dos in the database yet",
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
...
def set_done(todo_id: Annotated[
    int,
    typer.Argument(
        ...,
        help="The to-do item id",
    )
]) -> None:
    ...
    if error:
        ...
        typer.secho(
            f"""to_do #{todo_id} "{todo["Description"]}" was completed successfully""",
            fg=typer.colors.GREEN,
        )

@app.command(name="remove")
def remove(todo_id: Annotated[
    int,
    typer.Argument(
        ...,
        help="The to-do item id",
...
force: Annotated[
    ...
    typer.Option(
        ...
        "-f",
        is_flag=True,
        help="Force removing the to-do without confirmation",
    )
] = False) -> None:
    """Remove a to-do by  using the to-do ID"""
    todoer = get_todoer()
    ...
    def _remove():
        ...
        if error:
            ...
            typer.secho(
                f"""Removing to_do #{todo_id} "{todo["Description"]}" was completed successfully""",
                fg=typer.colors.GREEN,
            )
    ...
    if force:
        ...
        if delete:
            ...
            typer.secho("Operation cancelled", fg=typer.colors.BLUE)
